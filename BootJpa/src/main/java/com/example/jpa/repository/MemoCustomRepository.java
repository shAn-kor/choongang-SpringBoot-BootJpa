...
public interface MemoCustomRepository {
    ...
    List<Memo> memoJoinMember();List<Memo> mtoJoin1(long mno); //manyToOne조인 (N + 1문제) - 조인을 하기 위해 추가적인 select구문을 날리는 행위
    ...
    // queryDSL
    Memo dslSelect();
    List<Memo> dslSelect2();
    List<Memo> dslSelect3(String searchType, String searchName);
    List<Memo> dslJoin();
    Page<MemberMemoDTO> dslJoinPage(Pageable pageable, Criteria cri);
}
